    ...
    <div className="min-h-screen bg-slate-50 text-slate-900">
      <header className="border-b border-slate-200 bg-white/80 backdrop-blur sticky top-0 z-40">
        <div className="max-w-3xl mx-auto px-6 py-4 flex items-center justify-between">
          <div>
            <p className="text-xs font-bold uppercase tracking-[0.3em] text-slate-400">Event Detail</p>
            <h1 className="text-2xl font-serif text-slate-900">{event.title}</h1>
          </div>
      ...
      <main className="max-w-3xl mx-auto px-6 py-8 space-y-6">
        ...
        {event.mediaUrls && event.mediaUrls.length > 0 && (
          <section className="bg-white rounded-[2rem] border border-slate-200 p-6 shadow-sm space-y-4">
            <h2 className="text-sm font-bold uppercase tracking-[0.2em] text-slate-500">Event media</h2>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {event.mediaUrls.map((url) => (
                <div key={url} className="rounded-2xl overflow-hidden border border-slate-200 bg-slate-100">
        ...
        <section className="bg-white rounded-[2rem] border border-slate-200 p-6 shadow-sm space-y-4">
          <h2 className="text-sm font-bold uppercase tracking-[0.2em] text-slate-500">Attendees</h2>
          {attendees.length === 0 ? (
            ...
            <div className="flex flex-wrap gap-2">
              {attendees.map((attendee) => (
                <Link
                  ...
                  target="_blank"
                  className="px-3 py-1 rounded-full text-[11px] font-bold text-slate-600 bg-indigo-50 border border-indigo-100 hover:bg-indigo-100 transition"
                >
                  {attendee.name}
                </Link>
              ))}
